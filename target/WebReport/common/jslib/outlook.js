...
function b_writeStyle() {
  ...
  document.write('.button {width:300; color:#ffffff; text-align:center; font-family:arial;');
  document.write(' font-size:10pt; cursor:hand; border-width:3;');
  document.write(' border-style:outset; border-color:#ffffff; ');
  document.write('background-color:#0876E7;}');
  ...
  if (bt.ns4) {                 //draw OutlookBar for ns4
      ...
      document.write('<Layer name='+this.name+'_panel'+i+' width='+this.width);
       ...
       document.write('<Layer top=38 width='+this.width+'>');
        ...
        for (j=0;j<this.panels[i].img.length;j++) {
          ...
          document.write("<Font size=2 face=arial color=#336666>");
          document.write(this.panels[i].lbl[j]+"</FONT><BR>");
    ...
    for (i=0;i<this.panels.length;i++) {
      ...
      if (!bt.op5) {//IE????
        ...
        document.write('onClick="javascript:parent.frmright.location=\'/admanager/adbanner/notBusyNum.kemp\';'+this.v+'.showPanel('+i);
        document.write(');" style="position:absolute; left:0; top:0; width:');
        document.write((this.width-c)+'; height:'+(28-c)+';"><table border=0><tr><td><A href="#"');
...
function createOutlookBar(name,x,y,width,height,bgcolor,pagecolor) {
  ...
  this.buttonspace=68                     // distance of panel buttons
