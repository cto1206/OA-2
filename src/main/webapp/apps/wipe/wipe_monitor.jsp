...
<s:include value="/apps/pageHeader.jsp">
	...
	<table width="650px;" cellpadding="0" cellspacing="1" class="form" >
			...
			<tr>
					<td>一级审查</td>
					<td style="color:blue;">
						<s:if test="wipe.judgeSet[0].judgeDate != null">
							（<s:property value="wipe.judgeSet[0].user.rName"/>）已审批
						</s:if>
						<s:else>（<s:property value="wipe.judgeSet[0].user.rName"/>）待审</s:else>
					</td>
					<td>二级决裁</td>
					<td style="color:blue;">
		...
		<s:if test="wipe.judgeSet[3].judgeDate != null && wipe.judgeSet[4].user.rName!=null">
					<tr>
						<td>最终决裁</td>
						<td style="color:blue;">
...
<br>
<table width="650px;" cellpadding="0" cellspacing="1" class="form">
<tr><td colspan="5" align="center">指示事项列表</td></tr>
<tr>
    <td>审批人编号</td> <td>审批人名字</td><td>审批时间</td><td>指示事项</td>
</tr>
...
<table >
	<tfoot>
				<tr align="right"><td colspan="4" ><input type="button" value="返回" onclick="window.history.go(-1);"/></td></tr>
			</tfoot>
</table>
