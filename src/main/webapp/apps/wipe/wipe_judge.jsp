	...
	<tbody>
			...
			<s:if test="task.name == '四级合议'">
				<tr>
					<td>最终决裁</td>
					<td colspan="3">
					    <s:hidden name="topJudge"/>
						<s:textfield name="topJudge1" theme="simple" cssClass="UserSel" onclick="selUser('topJudge','topJudge1')"/>
					</td>
				</tr>
			</s:if>
			...
			<tfoot>
				<tr>
					<td colspan="4"><input type="button" value="批准" id="okBtn">&nbsp;&nbsp;<input
						type="button" value="驳回" id="cancelBtn" />
						</td>
			...
			if (val){
		 		$.post('<s:url action="ringi_checkCode" includeParams="none" namespace="/"/>', {"ringiShoCode":val} ,function(text){
		 			...
		 			if(obj.checked){
		 				...
		 				$("#wipe_confirm").submit();//.ajaxSubmit();
		 			}else{
